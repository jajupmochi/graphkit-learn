...
def generate_graphs(n_graphs: int, n_emb_dim: int = 2, seed: int = 42) -> List[nx.Graph]:
	"""
	Generate a list of random graphs with node labels containing coordinates in string format.
	"""
	...
	# Check the graph node label format:
...
def fit_model_either_version(
		use_global_env: bool,
		seed: int = 42, n_graphs: tuple[int, int] = (100, 50), n_emb_dim: int = 2,
		parallel: bool = False, ged_init_mode: str = 'eager'
) -> (np.array, float):
	"""
	...
	graphs1 = generate_graphs(n_graphs[0], n_emb_dim=n_emb_dim, seed=seed)
	graphs2 = generate_graphs(n_graphs[1], n_emb_dim=n_emb_dim, seed=seed)
	...
	model, matrix, stats = fit_model_ged(
		...
		output_dir=None,
		params_idx=None,
		use_global_env=use_global_env,  # Use which GEDEnv for each pair
		verbose=2,
		**fit_settings
	)
	print("Model:", model)
...
def print_stats_local_version(stats: dict):
	...
	if not pairwise_stats:
		print(f'{INFO_TAG}No pairwise stats available.')
		return
	...
	time_stats = {}
	for key in keys:
		if key not in pairwise_stats[0]:
			continue
...
def compare_ged_model_with_global_and_local_env_cross_matrix(
		seed: int = 42, n_graphs: tuple[int, int] = (100, 50), n_emb_dim: int = 2,
		parallel: bool = False, ged_init_mode: str = 'eager'
) -> (np.array, np.array):
	"""
	...
	"""
	cost_matrix_g, stats_g = fit_model_either_version(
		use_global_env=True,  # Use global GEDEnv for all pairs of graphs
		seed=seed, n_graphs=n_graphs, n_emb_dim=n_emb_dim, parallel=parallel,
		ged_init_mode=ged_init_mode
	)
	cost_matrix_l, stats_l = fit_model_either_version(
		use_global_env=False,  # Use local GEDEnv for each pair of graphs
		seed=seed, n_graphs=n_graphs, n_emb_dim=n_emb_dim, parallel=parallel,
		ged_init_mode=ged_init_mode
	)
...
if __name__ == '__main__':
	...
	compare_ged_model_with_global_and_local_env_cross_matrix(
		seed=seed, n_graphs=n_graphs, n_emb_dim=n_emb_dim, parallel=parallel,
		ged_init_mode=ged_init_mode
	)
...
#
# [info] The total run time for the GEDModel: 165.527 s / 0.001103511 s per pair.
#
...
#
# [info] The total run time for the GEDModel: 29.026 s / 0.000193509 s per pair.
# [info] env setting time: 0.025 s. (0.09% of total run time).
# [info] graphs adding time: 0.352 s / 0.000440472 s per graph. (1.21% of total run time).
# [info] ged computing time: 27.513 s / 0.000183417 s per pair. (94.78% of total run time).
#
#
...
# ### local version (GEDEnv created for each pair of graphs):
#
# [info] The total run time for the GEDModel: 77.923 s / 0.000519487 s per pair.
#
# ### global version (GEDEnv created once for the model):
#
# #### ✅ Using `LAZY_WITHOUT_SHUFFLED_COPIES` init option (computing costs when actually needed):
#
# [info] The total run time for the GEDModel: 5.806 s / 0.000038708 s per pair.
# [info] env setting time parallel: 1.374 s / 0.137399626 s per worker. (23.66% of total run time).
# [info] graphs adding time parallel: 5.109 s / 0.000638592 s per worker per graph. (87.99% of total run time). Time per worker: [0.36477065 0.39869857 0.43185973 0.57985783 0.57254052 0.50350499
#  0.61493111 0.48274899 0.60086203 0.55896163].
# [info] ged computing time parallel: 38.818 s / 0.000258788 s per pair. (668.57% of total run time).
#
#
# ### Conclusion:
# - With parallelization, the global version with `LAZY_WITHOUT_SHUFFLED_COPIES` init option is
#   77.923 / 5.806 = 13.41 times faster than the local version.
# - Without parallelization, the global version with `LAZY_WITHOUT_SHUFFLED_COPIES` init option is
#   165.527 / 29.026 = 5.71 times faster than the local version.
# - The global version with parallelization is 29.026 / 5.806 = 5.00 times faster than the global
