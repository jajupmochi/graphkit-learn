...
def run_xp(ds_name, output_file, unlabeled, mode, y_distance, ed_method):
	...
	)  # @todo: ged can not deal with sym and unsym labels.
	Gn = ds.graphs
	y_all = ds.targets
...
if __name__ == "__main__":
	...
	if len(sys.argv) > 1:
		...
		Dataset_list = [
			'Alkane_unlabeled', 'Acyclic', 'Chiral', 'Vitamin_D',
			'Steroid'
		]
		...
		for task in list(task_grid):
			...
			if not os.path.isfile(output_result):
				run_xp(
					task['dataset'], output_result, unlabeled, mode,
					task['distance'], task['edit_cost']
				)
