...
"""
	...
	This module allows to use a C++ library for edit distance between graphs (GedLib) with Python.
...
cdef extern from "src/GedLibBindAttr.hpp" namespace "pyged":
...
lib1 = cdll.LoadLibrary(
	os.path.dirname(os.path.realpath(__file__)) + '/lib/fann.2.2.0/libdoublefann.so'
)
lib2 = cdll.LoadLibrary(
	os.path.dirname(os.path.realpath(__file__)) + '/lib/libsvm.3.22/libsvm.so'
)
lib3 = cdll.LoadLibrary(
	os.path.dirname(os.path.realpath(__file__)) + '/lib/nomad.3.8.1/libnomad.so'
)
lib4 = cdll.LoadLibrary(
	os.path.dirname(os.path.realpath(__file__)) + '/lib/nomad.3.8.1/libsgtelib.so'
)
...
cdef class GEDEnvAttr:
	...
	def __cinit__(self, label_type: str = 'attr'):
		"""
	...
	def add_node(self, size_t graph_id, node_id, dict node_label):
		"""
	...
	def get_original_node_ids(self, graph_id):
		"""
			...
			.. note:: These functions allow to collect all the graph's informations.
		"""
		return [gid.decode('utf-8') for gid in
	...
	def get_num_graphs(self) -> size_t:
		"""
