"""
ged_model_global_env

The GEDModel class using a GEDEnv as a global environment inside the class.
...
class GEDModel(BaseEstimator):  # , ABC):
	...
	##########################################################################
	# The following is the paradigm to compute GED distance matrix, which is
	...
	def fit(self, X, y=None, **kwargs):
		...
		if self.parallel is None:
			...
			# GED environment.
			self._ged_env = self.create_and_setup_ged_env(
	...
	def compute_distance_matrix(self, Y=None, **kwargs):
		...
		if Y is None:
			...
			# Graphs copying will be done when loading the graphs into the GEDEnv.
...
#		This method is abstract and must be implemented by a subclass.
	...
	def _compute_self_distance_matrix(self, **kwargs):
		# Graphs were loaded into GEDEnv beforehand. No need to copy again.
	...
	def _compute_self_distance_matrix_series(self, **kwargs):
		...
		self.init_ged_env_and_method(self._ged_env, **{'ged_init_options': self.ged_init_options})
		...
		if self.verbose >= 2:
			iterator = tqdm(
				iterator, desc='Computing distance matrix',
				file=sys.stdout, total=len_itr
			)
		for i, j in iterator:
			gid1, gid2 = graph_ids[i], graph_ids[j]
			dis_matrix[i, j], _ = GEDModel.pairwise_ged_with_gids(
			...
			try:
				...
				if method == 'joblib':
					...
					def init_worker_self_metric_matrix(graphs):
						...
						g_ged_env = GEDModel.create_and_init_ged_env_for_parallel(g_graphs, **init_kwargs)
	...
	@staticmethod
	def _process_pair_worker(pair, shm_name, matrix_shape, compute_ged_func, **kwargs):
		...
		try:
			...
			distance, _ = compute_ged_func(i, j, **kwargs)
	...
	def _compute_cross_distance_matrix_series(
		...
		if self.verbose >= 2:
			iterator = tqdm(
				iterator, desc='Computing distance matrix', file=sys.stdout, total=len_itr
			)
		# for i, j in get_iters(
		# 		iterator, desc='Computing distance matrix',
		# 		file=sys.stdout, verbose=(self.verbose >= 2), length=len_itr
		# ):
		for i_f, j_t in iterator:
			gid_f, gid_t = i_f, j_t
			dis_matrix[j_t, i_f], _ = self.pairwise_ged_with_gids(
			...
			try:
				...
				if method == 'joblib':
					...
					def init_worker_cross_metric_matrix(graphs_f, graphs_t):
						...
						g_graphs_t = graphs_t
						g_ged_env = GEDModel.create_and_init_ged_env_for_parallel(
	...
	def create_and_init_ged_env_for_parallel(graphs: list[nx.Graph], **kwargs):
		...
		ged_env = GEDModel.create_and_setup_ged_env(graph=graphs[0], **kwargs)
		# print(f'[{multiprocessing.current_process().name}] ')
		# print(ged_env)

		# Add all graphs to the environment:
		GEDModel.add_graphs_to_ged_env(graphs, ged_env, verbose=0, **kwargs)
		# print('fnished adding graphs to the GEDEnv in worker.')
		# print(ged_env.get_all_graph_ids())
		GEDModel.init_ged_env_and_method(ged_env, **kwargs)
	...
	def _process_pair_worker_unified(
		...
		try:
			...
			# Compute distance using the function reference
			distance, _ = compute_ged_func(i_f, j_t, **kwargs)
		...
		return i_f, j_t, distance  # Return for progress tracking
	...
	def pairwise_ged_with_gids_parallel(
		...
		dis, _ = GEDModel.pairwise_ged_with_gids(
	...
	def init_ged_env_and_method(ged_env, **kwargs):
		...
		if init_options.startswith('EAGER_'):
			print(f'Starting eager label cost computing. This may take a while...')
			ged_env.init(init_options)
			print(f'Eager label cost computing finished.')
		else:
			ged_env.init(init_options)
