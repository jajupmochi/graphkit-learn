...
class MedianGraphEstimatorCML(object): # @todo: differ dummy_node from undifined node?
	...
	def __init__(self, ged_env, constant_node_costs):
		...
		self._node_del_cost = ged_env.get_node_del_cost(ged_env.get_node_label(1, to_dict=False))
		self._node_ins_cost = ged_env.get_node_ins_cost(ged_env.get_node_label(1, to_dict=False))
		self._labeled_edges = (ged_env.get_num_edge_labels() > 1)
		self._edge_del_cost = ged_env.get_edge_del_cost(ged_env.get_edge_label(1, to_dict=False))
		self._edge_ins_cost = ged_env.get_edge_ins_cost(ged_env.get_edge_label(1, to_dict=False))
	...
	def run(self, graph_ids, set_median_id, gen_median_id):
		...
		for graph_id in graph_ids:
			...
			graphs[graph_id] = self._ged_env.get_nx_graph(graph_id)
...
#			sample_initial_medians_(graph_ids, initial_medians)
	...
	def _compute_medoid(self, graph_ids, timer, initial_medians):
		...
		if self._parallel:
			...
			initial_medians.append(self._ged_env.get_nx_graph(medoid_id)) # @todo
			...
			for g_id in graph_ids:
				...
				for h_id in graph_ids: # @todo: this can be faster, only a half is needed.				
					...
					if nb_nodes_g <= nb_nodes_h or not self._sort_graphs:
						self._ged_env.run_method(g_id, h_id) # @todo
...
# 		print('----------------------------')
		...
		all_labels = self._ged_env.get_all_node_labels()
			...
			if len(node_labels) > 0:
				...
				for label1 in all_labels:
					...
					for label2 in node_labels:
						fi += self._ged_env.get_node_cost(label1, label2) # @todo: check inside, this might be slow
...
# 					nx.set_node_attributes(median, {i: median_label})
	...
	def _update_edges(self, graphs, median):
		...
		all_labels = self._ged_env.get_all_edge_labels()
			...
			for j in range(i + 1, nx.number_of_nodes(median)):
				...
				if self._labeled_edges and len(edge_labels) > 0:
					...
					for label2 in edge_labels:
						fij0 += self._ged_env.get_edge_cost(SpecialLabel.DUMMY, label2)
...
# 			print(new_node_map.forward_map, new_node_map.backward_map
	...
	def _increase_order(self, graphs, median):
		...
		best_config = {}
		best_label = self._ged_env.get_node_label(1, to_dict=True)
		increased_order = False
	...
	def _compute_insertion_delta_constant(self, inserted_nodes, best_config, best_label):
		...
		best_config.clear()
		best_delta = 0.0
		node_rel_cost = self._ged_env.get_node_rel_cost(self._ged_env.get_node_label(1, to_dict=False), self._ged_env.get_node_label(2, to_dict=False))
	...
	def _compute_insertion_delta_generic(self, inserted_nodes, best_config, best_label):
		...
		for node_label in initial_node_labels:
			...
			config = {}
			for graph_id, _ in inserted_nodes.items():
				config[graph_id] = tuple((np.inf, self._ged_env.get_node_label(1, to_dict=False)))
